...
TroykaTextLCD::TroykaTextLCD(uint8_t pinBacklight, uint8_t i2cAddress, TwoWire* wire) {
...
void TroykaTextLCD::setContrast(uint8_t contrast) {
    ...
    command(LCD_EX_POWICONCONTRASTH | LCD_ICON_ON | LCD_BOOST_ON | ((contrast >> 4) & 0x03));
    setNormalFunction();
}
...
void TroykaTextLCD::command(uint8_t value) {
    send(value, 0x00);
}
